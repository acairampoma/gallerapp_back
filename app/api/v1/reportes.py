    ...
    try:
        ...
        if tipo == "gallos":
            query = text(f"""
                SELECT json_agg(
                    json_build_object(
                        'ranking', ROW_NUMBER() OVER (ORDER BY efectividad DESC, peleas_ganadas DESC),
                        'id', g.id,
                        'nombre', g.nombre,
                        'raza_id', g.raza_id,
                        'codigo_identificacion', g.codigo_identificacion,
                        'total_peleas', COUNT(p.id),
                        'peleas_ganadas', COUNT(CASE WHEN p.resultado = 'ganada' THEN 1 END),
                        'efectividad', ROUND(COALESCE(COUNT(CASE WHEN p.resultado = 'ganada' THEN 1 END)::numeric / NULLIF(COUNT(p.id), 0) * 100, 0), 1),
                        'ingresos_estimados', COUNT(CASE WHEN p.resultado = 'ganada' THEN 1 END) * 3000
            ...
            query = text(f"""
                SELECT json_agg(
                    json_build_object(
                        'ranking', ROW_NUMBER() OVER (ORDER BY efectividad DESC),
                        'id', padre.id,
                        'nombre', padre.nombre,
                        'raza_id', padre.raza_id,
                        'total_hijos', COUNT(DISTINCT h.id),
                        'peleas_ganadas_hijos', COUNT(CASE WHEN p.resultado = 'ganada' THEN 1 END),
                        'efectividad', ROUND(COALESCE(COUNT(CASE WHEN p.resultado = 'ganada' THEN 1 END)::numeric / NULLIF(COUNT(p.id), 0) * 100, 0), 1)
                    ) ORDER BY efectividad DESC
                ) as ranking_data
                FROM gallos padre
        ...
        return {
            ...
            "data": result.ranking_data if result and result.ranking_data else [],
            "timestamp": time.time()
        }
